...
record_path = os.path.join(output_dir, f"Packers_and_Movers_Estimation_{timestamp}.mp4")
...
product_colors = {
    "Cooler": (128, 0, 128),          # Violet
    "Furniture": (75, 0, 130),        # Indigo
    "Iron_Box": (0, 0, 255),          # Blue
    "Laptop": (0, 128, 0),            # Green
    "Microwave_Oven": (255, 255, 0),  # Yellow
    "Mixture": (255, 165, 0),         # Orange
    "Phone": (255, 0, 0),             # Red
    "Refridgerator": (148, 0, 211),   # Dark Violet
    "Remote": (0, 0, 139),            # Dark Blue
    "Standing_Fan": (0, 100, 0),      # Dark Green
    "Tele_Vision": (139, 69, 19),     # Saddle Brown
    "Vacuum_Cleaner": (160, 82, 45),  # Sienna
    "Washing_Machine": (128, 128, 0)  # Olive
}
...
while True:
    ...
    for det in detections:
        ...
        color = product_colors.get(label_name, (100, 100, 100))  # use prod directly
    ...
    for idx, prod in enumerate(sorted(product_counter)):
        ...
        color = product_colors.get(prod, (100, 100, 100))  # fallback if not found
        cv2.putText(frame, display_text, (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
    ...
    # Shadow (offset by 1 pixel for depth)
    cv2.putText(frame, wm1, (x1 + 1, y1 + 1), font, scale, shadow_color, thickness + 1)
    cv2.putText(frame, wm2, (x2 + 1, y2 + 1), font, scale, shadow_color, thickness + 1)

    # Gold text over shadow
    cv2.putText(frame, wm1, (x1, y1), font, scale, gold_color, thickness)
    cv2.putText(frame, wm2, (x2, y2), font, scale, gold_color, thickness)
    ...
    # --- Initialize recorder dynamically after avg_fps is stable ---
    if record and not writer_initialized and len(fps_list) >= 20:
        print(f"[📹 RECORDER INITIALIZED] Using avg_fps: {avg_fps:.2f}")
        recorder = cv2.VideoWriter(record_path, cv2.VideoWriter_fourcc(*'mp4v'), avg_fps, (resW, resH))
